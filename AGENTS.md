...
- Backend tests: `pip install -r requirements-dev.txt`, then `python -m pytest` from `backend/`. They cover pure logic only (JSON salvage parser, LaTeX, slide-skipping) — there's no route coverage, so verify wiring by booting the server and curling `/api/health`, `/api/providers/presets`, etc. Lint with `ruff check .` from `backend/`; it's scoped to ruff's default `E4/E7/E9/F` (unused imports, undefined names) and ignores `E712` because SQLAlchemy boolean filters need `Column == True`. `conftest.py` must keep redirecting `DATABASE_URL` at a temp dir: `app/__init__.py` imports `app.main`, so any `app.` import boots the app and migrates whatever DB it finds.
...
- `POST /api/render` (`routers/render.py`) is stateless on purpose: it stores nothing, holds no credentials, and makes no outbound requests, which is why it can be unauthenticated. Adding any of those three breaks the argument. Inputs are bounded (`MAX_UPLOAD_MB`, DPI 72–300, streamed upload, client filename never used as a path) and blocking work goes through `run_in_threadpool`.
- The slide fan-out hands worker threads a detached `ProviderConfig` snapshot, **never the `Provider` ORM row**. `expire_on_commit` is on, so each per-slide commit expires the row and an off-thread attribute read would lazy-load it — running SQL on the main thread's Session. That killed ~3 runs in 10 mid-generation. Snapshot anything else a worker needs from the DB the same way.
...
- Vision capability is decided by `_is_vision_capable` in `routers/providers.py` (provider type, then model-id prefix) — that function is the only place, so add new vision models there. A duplicate `VISION_CAPABLE_*` pair in `app/config.py` was imported by nothing and had drifted wrong; it's been removed.
